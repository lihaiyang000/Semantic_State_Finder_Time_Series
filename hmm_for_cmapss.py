...
#choose one for refinement or initial step
data = DataInit('E:\works\show\PythonHMM\data_result_ten_seconds.csv', (), (), [], 'E:\works\show\PythonHMM\data_rle_55_4_ten.csv', 'E:\works\show\PythonHMM\data_symbol_55_4.csv')
#data = UpdateHmm('./CMAPSS_data_HPC/first_sequence.log')
...
def write_result_part2(current_d, current_t, current_oldState, prevd, prevs, prePattern):
    ...
    s = s.rstrip(',')
    s = s + str('\n')
    file = open('E:\works\show\PythonHMM\esult_first.log', 'a')#需要更改地址
    file.write(s)
    file.close()
    if t-d == 0:
        print "finish!!!!"
    ...
    def relativeErr(self, beginTime, endTime, pattern):
        ...
        for i in xrange(0, length):
            if math.fabs(pattern - float(patternValuesArr[i])/float(patternValuesArr[i])) > self.segma:
                if math.fabs((pattern - float(patternValuesArr[i]))/pattern) > 0.005:
                    relativeError = 10
                    break
                    ...
                    if self.relativeErr(t-d, t, pattern) > self.segma:
                        ...
                        if d == t:
                            ...
                            for oldState in self._states:
                                ...
                                #update time
                                #newTemp = delta['delta' + str(t-d) + '(' + str(oldState) + ')'] * self.trans_prob(oldState-1 , state-1) * self.emit_prob(state, t-d, t, pattern)   
                                
                                # print delta['delta' + str(t-d) + '(' + str(oldState) + ')'], "========="
                                # print self.trans_prob(oldState-1,state-1), "============"
                                # print self.emit_prob(state, t-d,t,pattern), "============"
                                # print "newTemp is", newTemp, "d is",d, "t is", t, "oldState is", oldState, "state is",state
...
ss = Model(a1, b1, e1, c1, d1, 0.005)
prevd, prevs, prePattern, delta = ss.ex_decoding(e1, 'E:\works\show\PythonHMM\data_result_ten_seconds.csv', 'E:\works\show\PythonHMM\data\data_symbol_55_4.csv')
print prevd
print prevs
print prePattern
print delta
d, t, oldState = ss.write_result(prevd, prevs, prePattern, delta, 192)
write_result_part2(d,t,oldState,prevd,prevs,prePattern)
