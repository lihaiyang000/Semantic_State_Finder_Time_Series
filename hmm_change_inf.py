...
data = DataInit('/home/hao/newworkspaces/StateFinder-master/StateFinder/input-example/data.csv', (), (), [], '/home/hao/newworkspaces/StateFinder-master/data35-rle.csv')
...
def write_result_part2(current_d, current_t, current_oldState, prevd, prevs):
    ...
    while j >= 0:
        if (prevs[j][3] == t) and (prevs[j][2] == maxState) and (prevs[j][0] == d):
            ...
            break
        j = j -1
    resultList = [d , t , maxState , oldState]    
    s = ''
    n = 0
    ...
    s = s.rstrip(',')
    s = s + str('\n')
    file = open('./test_datatest3.log', 'a')
    file.write(s)
    file.close()
    if t-d == 0:
        print "finish!!!!"
    else:
        write_result_part2(d, t, oldState, prevd, prevs)
...
class Model(object):
    ...
    def __init__(self, states, symbols, start_prob, trans_prob):
        self._states = states
    ...
    #计算分为两种情况:1.为初始概率,考虑1到t时刻的情况.所有的Pattern的均值.
    #2.d到t时刻多个Pattern的均值        
    def emit_prob(self, state, indexBegin, indexEnd):
        print "emit_prob=================="
        print "t-d is",indexBegin, "t is",indexEnd
        ...
        plusValue = (stateMaxPoint - stateMinPoint)/9
        #interval = valuesArr[index]
        # 不在state范围内不进行计算
        if (interval > stateMaxPoint):
          bInter = stateMaxPoint
          eInter = float('Inf')
          outProb, err = integrate.quad(norm_function, bInter, eInter, (stateMeans, std))
          print "max to Inf", outProb
          print "emit_prob=================="
          return outProb
        elif (interval < stateMinPoint):
          bInter = float('-Inf')
          eInter = stateMinPoint
          outProb, err = integrate.quad(norm_function, bInter, eInter, (stateMeans, std))
          print "-Inf to min", outProb
          print "emit_prob=================="
          return outProb
        else:
          bInter = 0
    ...
    def ex_decoding(self, sequence):
        ...
        for state in self._states:
            newStateTemp = self.start_prob(state) * self.emit_prob(state, 0, 1)
            delta['delta' + str(1) + '(' + str(state) + ')'] = newStateTemp
            print delta['delta' + str(1) + '(' + str(state) + ')']
            if newStateTemp > currentStateTemp:
                currentStateTemp = newStateTemp
                stateResult = state
        print stateResult
        prevd.append([1,stateResult,1])
        for t in xrange(2, numOfTime+1):
            for state in self._states:
                delta['delta' + str(t) + '(' + str(state) + ')'] = 0
                for d in xrange(1, t):
                    # if d == t:
                    #     temp = self.start_prob(state) * self.emit_prob(state, t-d, t)# how to cal the prob??
                    #     print "==============================="
                    #     print "d equal to t"
                    #     print "state is",state,"d is",d,"t is",t
                    #     print temp
                    #else:
                    for oldState in self._states:
                        ...
                        print [d,t]
                        emit_prob = self.emit_prob(state,t-d,t)
                        newTemp = delta['delta' + str(t-d) + '(' + str(oldState) + ')'] * self.trans_prob(oldState , state) * emit_prob
                        # s = str(d) + str(',') + str(t) + str(',') + str(oldState) + str(',') + str(state) + str(',') + str(delta['delta' + str(t-d) + '(' + str(oldState) + ')']) + str(',') + str(self.trans_prob(oldState , state)) + str(',') + str(emit_prob) + str('\n')
                        # file = open('/home/hao/py-work/PythonHMM/hmm_logs/hmm_td_unequal.log','a')
                        # file.write(s)
                        # file.close()
                        ...
                        print "d < t=============================="
                    temp = currentTemp
                print "temp dangqianzhi:",temp
                # s1 = str(t) + str(',') + str(state) + str(',') + str(temp) + str(',') + str(delta['delta' + str(t) + '(' + str(state) + ')']) + str('\n')
                # fileTemp = open('/home/hao/py-work/PythonHMM/hmm_logs/hmm_final_temp_vs_delta_t.log','a')
                # fileTemp.write(s1)
                # fileTemp.close()               
                currentTemp = 0
                if temp > delta['delta' + str(t) + '(' + str(state) + ')']:
                    print "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
                    print "we got d and t is", [d,t]
                    print temp
                    print "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
                    delta['delta' + str(t) + '(' + str(state) + ')'] = temp
                    # s2 = str(t) + str(',') + str(state) + str(',') + str(delta['delta' + str(t) + '(' + str(state) + ')']) + str('\n')
                    # fileDelta = open('/home/hao/py-work/PythonHMM/hmm_logs/hmm_delta.log','a')
                    # fileDelta.write(s2)
                    # fileDelta.close() 
                    prevd.append([d,state,t])
        ...
        return prevd, prevs, delta


    def write_result(self, prevd, prevs, delta, length):
        states = self._states
        maxProb = 0.0
        maxState = 0
        oldState = 0
...
ss = Model(a1, b1, c1, d1)
prevd, prevs, delta = ss.ex_decoding(e1)
print prevd
print prevs
d, t, oldState = ss.write_result(prevd, prevs, delta, 114)
write_result_part2(d,t,oldState,prevd,prevs)
